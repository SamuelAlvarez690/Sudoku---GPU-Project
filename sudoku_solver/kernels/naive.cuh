...
//
//  MAX_ITER guard: prevents pathological puzzles from hanging
//  the entire GPU. Threads that hit the limit mark the puzzle
//  as unsolved rather than running forever.
// ============================================================

static const int NAIVE_MAX_ITER = 2000000;  // ~2M backtracks before giving up
...
// iter is passed by pointer so recursion shares the counter across all frames
__device__ bool naive_solve(int* board, int* iter) {
    if (++(*iter) > NAIVE_MAX_ITER) return false;
    ...
    for (int val = 1; val <= 9; val++) {
        if (naive_is_valid(board, pos, val)) {
            board[pos] = val;
            if (naive_solve(board, iter)) return true;
            board[pos] = 0;
        }
    }
    return false;
...
__global__ void naive_sudoku_kernel(const int* __restrict__ d_puzzles, int* __restrict__ d_solutions, bool* __restrict__ d_solved, int N)
{
    ...
    int iter = 0;
    d_solved[tid] = naive_solve(board, &iter);
